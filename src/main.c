...
void update_uniforms(GameState* state) {
    ...
    player_eye.z += 0.5;
    {
        ...
        */
        ...
        if (glfwGetMouseButton(state->window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
            ...
            if(shoot_timer < 0.0f) { shoot_timer = shoot_delay; }
        }
        if(shoot_timer > 0.0f) { state->crosshair_color = vec4_new(1.0, 0.0, 0.0, 0.5); }
        ...
        const vec4 death_flash_color = vec4_new(0.0, 0.0, 1.0, 0.5);
...
void recreate_swapchain(GameState* state);
...
global void cursor_position_callback(GLFWwindow* window, double xpos, double ypos) {
...
void create_shader_modules(GameState* state) {
    ...
    char* file_names[] = {"../level.vert.spv", "../level.frag.spv", "../enemy.vert.spv", "../enemy.frag.spv", "../crosshair.vert.spv", "../crosshair.frag.spv"};
...
void create_crosshair_graphics_pipeline(GameState* state) {
    ...
    create_graphics_pipeline(
        ...
        0,
        &pipeline_options,
        &state->crosshair_pipeline.layout,
        state->swapchain_format,
        state->depth_image_format,
        &state->crosshair_pipeline.pass,
        &state->crosshair_pipeline.pipeline
    );
}
...
void create_enemy_buffers(GameState* state, LoaderState* loader) {
    ...
    create_device_local_and_staging_buffer(
        ...
        VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
        &state->enemy_color_buffer.buffer,
        &state->enemy_color_buffer.memory,
        &state->enemy_color_staging_buffer.buffer,
        &state->enemy_color_staging_buffer.memory
    );
}
...
void create_crosshair_buffers(GameState* state) {
    create_device_local_and_staging_buffer(
        ...
        VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
        &state->crosshair_color_buffer.buffer,
        &state->crosshair_color_buffer.memory,
        &state->crosshair_color_staging_buffer.buffer,
        &state->crosshair_color_staging_buffer.memory
    );
}
...
void load_level_model(GameState* state, LoaderState* loader) {
    loader->level_path = "../test_3.level";
...

    {
        ...
        state->enemy_alive_count = state->max_enemy_count;
        ...
        for(u32 index = i; index < state->enemy_alive_count; index += 1) {
        ...
        const i32 enemy_default_health = 3;
